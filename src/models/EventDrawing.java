			...
			if (clickEvent.getClickCount() == 2) {
				...
				if (!contextMenuIsOpen) {
					...
					item2.setOnAction(new EventHandler<ActionEvent>() {
						@Override
						public void handle(ActionEvent e) {
							ArrayList<Calendar> calendars = PersonInfo
									.getPersonInfo().getAllCalendars();
							...
							HeaderController.getController()
									.drawEventsForWeek();
							System.out.println(eventName + " is deleted!");
						}
					});
					contextMenu.getItems().addAll(item1, item2);
					contextMenu.show(eventRectangle, clickEvent.getScreenX(),
					...
					contextMenu.focusedProperty().addListener(
							new ChangeListener<Boolean>() {
								...
								public void changed(
									...
									if (newPropertyValue) {
										System.out
												.println("ContextMenu on focus");
									} else {
										System.out
												.println("ContextMenu out focus");
										contextMenu.hide();
